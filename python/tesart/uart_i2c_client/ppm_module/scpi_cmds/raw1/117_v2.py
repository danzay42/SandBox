...
## Variables
visa_address = 'TCPIP0::192.168.0.101::inst0::INSTR'
...
port1_power_level = -24.65
port1_att = 15.0

port2_power_level = -25
port2_att = 20.0

port3_power_level = -15
...
# Весь скрипт относится к пункту 1.1.9.2 методики
# Функция ниже относится к пункту 1.1.7.3 методики
def instr_configuring():
    ...
    send2instr(':CALCulate1:CUSTom:DEFine "%s","%s","%s"' % ('Noise Figure', 'Noise Figure Converters', 'NF'))
    send2instr(':CALCulate1:CUSTom:DEFine "%s","%s","%s"' % ('DUTNPD', 'Noise Figure Converters', 'DUTNPD'))

    send2instr(':DISPlay:WINDow1:TRACe1:FEED "%s"' % ('Noise Figure'))
    send2instr(':DISPlay:WINDow1:TRACe2:FEED "%s"' % ('DUTNPD'))
    ...
    send2instr(':SENSe:FREQuency:CENTer %G' % (center_freq))
    send2instr(':SENSe:FREQuency:SPAN %G' % (span))
    ...
    send2instr(':SENSe:NOISe:BWIDth:RESolution %G' % (noise_rbw))
    send2instr(':SENSe:NOISe:AVERage:COUNt %d' % (avg_count))
    send2instr(':SENSe:NOISe:AVERage:STATe %d' % (1))
    send2instr(':SENSe:NOISe:RECeiver %s' % ('NOISe'))
    send2instr(':SENSe:NOISe:GAIN %d' % (gain))
    send2instr(':SENSe:NOISe:TEMPerature:AMBient %G' % (temperature))

    send2instr(':CALCulate1:MEASure1:FORMat %s' % ('MLOG'))
    
    send2instr(':CALCulate1:MEASure2:FORMat %s' % ('MLOG'))
    send2instr(':CALCulate1:MEASure2:SMOothing:STATe %d' % (1))
    send2instr(':CALCulate1:MEASure2:SMOothing:APERture %G' % (2.0))
    
    step_label.config(justify=LEFT, text="Конфигурация ВАЦ прошла успешно.\nПодключите измеритель мощности к третьему порту и нажмите \"Далее\"")
    
    next_button.config(state=NORMAL)


# Function for configuring power sensor and calibrating power source (step 3)
# Функция ниже относится к пункту 1.1.7.4 методики
...
# Функция ниже относится к пункту 1.1.7.5 методики
def smart_cal_configuring():
    global max_state, max_cal_step
    
    step_label.config(text=step_descriptions[3])

    send2instr(':SENSe:NOISe:CALibration:METHod "%s"' % ('Scalar'))
    send2instr(':SENSe:NOISe:CALibration:RMEThod "%s"' % ('PowerMeter'))
...
# Функция ниже относится к пункту 1.1.7.10 методики
def marker_meas():
    ...
    send2instr(':CALCulate:MARKer1:FUNCtion:SELect %s' % ('MAXimum'))
    ...
    result_label.config(text='Коэффициент шума: %s' % (result))
    result_label.place(x=10, y=60)

    step_label.config(justify=LEFT, text="Измерение коэффициента шума прошло успешно")
...
def change_state():
    print('\n')
    global cur_state, max_state, steps, instr, VNA_VISA_address, channelNumber
...
root.title("Калибровка для измерения коэффициента шума приёмного тракта")
