...
port2_power_level = -10
port2_att = 20.0

port3_power_level = -10
...
# Функция ниже относится к пункту 1.1.6.4 методики
def instr_configuring():
    ...
    send2instr(':DISPlay:WINDow1:STATe %d; *WAI' % (1))
    send2instr(':CALCulate:PARameter:DELete:ALL; *WAI')

    send2instr(':CALCulate1:CUSTom:DEFine "%s","%s","%s"; *WAI' % ('116_measure', 'Scalar Mixer/Converter', 'Opwr'))
    send2instr(':DISPlay:WINDow1:TRACe1:FEED "%s"; *WAI' % ('116_measure'))
    ...
    send2instr(':SENSe:MIXer:LO1:FREQuency:MODE %s' % ('FIXED'))
...
# Функция ниже относится к пункту 1.1.6.5
def power_sens_configuring():
    ...
    justsend2instr(':SOURce:POWer:CORRection:COLLect:ACQuire %s,"%s","%s"' % ('PMETer', 'ASENSOR', 'Port 3'))
...
# Функция ниже относится к пункту 1.1.6.6
...
# Функция ниже относится к пункту 1.1.6.11
def first_measure():
    ...
    send2instr(':CALCulate:MARKer1:FUNCtion:DOMain:USER:STARt %G' % (output_freq_start))
    send2instr(':CALCulate:MARKer1:FUNCtion:DOMain:USER:STOP %G' % (output_freq_stop))
...
# Функция ниже относится к пункту 1.1.6.12
def second_measure():
